...
export default function AddReviewRestaurant(props) {
  ...
  const updateRestaurant = () => {
    ...
    restaurantRef.get().then((response) => {
      ...
      const raitingTotal = restaurantData.raitingTotal + raiting;
      const quantityVoting = restaurantData.quantityVoting + 1;
      const raitingRestult = raitingTotal / quantityVoting;

      restaurantRef
        .update({
          rating: raitingRestult,
  ...
  return (
    <View style={styles.viewBody}>
      ...
      <Loading isVisisble={isLoading} text="Enviando comentario" />
    </View>
  );
}
